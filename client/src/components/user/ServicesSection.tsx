...
export const ServicesSection = ({ settings }: ServicesSectionProps) => {
  ...
  const getFieldValue = (
    fieldsData: Array<{ name: string; value: any }>,
    fieldName: string
  ) => {
    ...
    if (fieldsDataField && Array.isArray(fieldsDataField.value)) {
      ...
      const field = fieldsDataField.value.find(
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Explore our comprehensive course offerings designed to help you
            succeed
          </p>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="flex gap-6 overflow-x-auto pb-4 hide-scrollbar">
              {courses.map((course, index) => {
                const courseName =
                  getFieldValue(course.fieldsData, "courseName") || "Course";
                const description =
                  getFieldValue(course.fieldsData, "description") ||
                  "No description available";
                const price =
                  getFieldValue(course.fieldsData, "fees") ||
                  getFieldValue(course.fieldsData, "price") ||
                  0;
                const duration =
                  getFieldValue(course.fieldsData, "duration") || 0;
                const category =
                  getFieldValue(course.fieldsData, "category") || "General";
                const courseImage = getFieldValue(
                  course.fieldsData,
                  "courseImage"
                );
                const fullImageUrl = courseImage
                  ? courseImage.startsWith("http")
                    ? courseImage
                    : `http://localhost:4070${courseImage}`
                  : null;
                const gradients = [
          ...
          <div className="text-center py-16 bg-white rounded-2xl border-2 border-dashed border-gray-200">
            ...
            <p className="text-gray-500">
              Courses will appear here once added through Course Management
            </p>
          </div>
        )}
      </div>
