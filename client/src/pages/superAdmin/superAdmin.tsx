...
import { defaultStudentFields, defaultCourseFields, defaultBranchFields, defaultExamResultFields } from './constants'
...
const SuperAdminPanel = () => {
    ...
    const { studentFields, courseFields, branchFields, examResultFields, loading, error } = useSelector(
    ...
    useEffect(() => {
        ...
        const loadFormConfigurations = async () => {
            ...
            try {
                ...
                const [studentData, courseData, branchData, examResultData] = await Promise.all([
                    loadFormData('student'),
                    loadFormData('course'),
                    loadFormData('branch'),
                    loadFormData('examResult')
                ])
                ...
                if (examResultData?.data) {
                    dispatch(setExamResultFields(convertFieldsFromBackend(examResultData.data.fields)))
                } else {
                    dispatch(setExamResultFields(defaultExamResultFields))
                }
    ...
    const getCurrentFields = () => {
        ...
        switch (activeForm) {
            ...
            case 'examResult':
                return examResultFields.length > 0 ? examResultFields : defaultExamResultFields
            default:
                return studentFields.length > 0 ? studentFields : defaultStudentFields
        }
    }
    ...
    const setCurrentFields = (fields: FieldConfig[]) => {
        ...
        switch (activeForm) {
            ...
            case 'branch':
                ...
                break
            case 'examResult':
                dispatch(setExamResultFields(fields))
                break
            default:
                dispatch(setStudentFields(fields)) // Fallback to student fields
        }
    }
